...
This directory documents **how FDDC is trained**, so the method is transparent and reproducible in
principle. FDDC is implemented on top of the public **Holosoma** framework (Amazon FAR, Apache-2.0);
what is *specific to FDDC* is a small set of files — the deployable dynamic-CoM observation and the
...
## What is FDDC-specific (method → file)

Everything else in a Holosoma checkout is upstream; FDDC lives in these files (all under
...
- **Holosoma** (Apache-2.0): https://github.com/amazon-far/holosoma — `pip install -e src/holosoma`
  (the FDDC extras `[unitree,booster]` are for real-robot deploy only and are *not* needed for training).
...
FDDC checkpoint (`model_0262000.pt`, step 262k) was trained on this set; checkpoints are selected on
...
- **Heavy stack.** Training needs IsaacSim (NVIDIA EULA) + multiple GPUs (the released run used 8× RTX
  ...
  of this.
- **Version drift.** The `fddc_src/` files are FDDC's modifications to Holosoma at upstream commit
  ...
  on a newer upstream, minor adaptation may be needed — the known trade-off of shipping the FDDC layer
